import numpy as np
from utils import deduct_balancing_fee, find_index_of_min

class DynamicMinimumStrategy:
    def distribute(investments_amount, initial_APYs):
        investments = [0.0] * len(initial_APYs)

        investments[0] = investments_amount
        
        distribution = DynamicMinimumStrategy.balance(investments, initial_APYs)
